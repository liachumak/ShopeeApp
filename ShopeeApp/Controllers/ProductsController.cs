...
public class ProductsController : Controller
{
    ...
    public async Task<IActionResult> Edit(ProductEditViewModel model, [FromForm] List<string> ImageOrder, [FromForm] List<int> DeletedImageIds)
    {
        ...
        int nextImageId = product.Images.Max(i => (int?)i.Priority) ?? 0;
        ...
        foreach (var item in ImageOrder)
        {
            ...
            {
                // Нові фото додаються в тому ж порядку, в якому вони були завантажені
                // Тобто беремо наступне з product.Images з Priority == 0
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        // Перевірка на "замовлення" чи історію, яку не можна порушити (на майбутнє)
